...
local function player()
	local pos = memory.readbyte(0x300) 
	local cx = memory.readbyte(camx) + (memory.readbyte(camx + 0x10) * 256)
	local cy = memory.readbyte(camy) + (memory.readbyte(camy + 0x10) * 256)
	local x = memory.readbyte(px) + (memory.readbyte(px + 0x10) * 256) - cx
	local y = memory.readbyte(py) + (memory.readbyte(py + 0x10) * 256) - cy
	local pyrad = 0x1C
	...
	gui.box(x - (pxrad / 2), y, x + (pxrad / 2), y - pyrad,"#0000FF35","#0000FFFF")
end
...
local function enemy()
	...
	for i = 6, 14, 1 do
		etype = memory.readbyte(0x300 + i)
		local fill = "#FF000040"
		local outl = "#FF0000FF"
		
		if etype ~= 0xFF then
			local x = memory.readbyte(ex + i) + (memory.readbyte(ex +  i + 0x10) * 256) - cx
			local y = memory.readbyte(ey + i) + (memory.readbyte(ey +  i + 0x10) * 256) - cy
			
			box = lookup(etype,i)
			
			gui.box(x - box[1],y-box[2],x-box[1]+box[3] - pxrad,y-box[2]+box[4],fill,outl)
